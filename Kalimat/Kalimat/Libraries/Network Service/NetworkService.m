...
#define POST @"Post"
#define GET @"Get"
#define BASE_URL @"http://api.hindawi.org/v1/kalimat/"
...
+ (NSMutableURLRequest *)requestWithParameters:(NSDictionary *)urlParameters andBaseURL:(NSString *)baseURL{
	...
	NSString *urlString = [[NSString stringWithString:baseURL] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
	//NSLog(@"%@",urlString);
    ...
    //urlObject=[NSURL URLWithString:[urlString stringByAddingPercentEscapesUsingEncoding: NSUTF8StringEncoding]];
	...
	//[urlRequest addValue:@"android" forHTTPHeaderField:@"User-Agent"];
	return urlRequest;
}
    ...
    if (urlParameters !=nil) {
        ...
        for (id param in urlParameters) {
            if ([param isKindOfClass:[NSString class]] && [[urlParameters objectForKey:param] isKindOfClass:[NSString class]]) {
                ...
                NSArray *arrayParams = [urlParameters objectForKey:param];
                NSMutableString *xml=[NSMutableString stringWithFormat:@"<plist><array>"];
                for (id subParam in arrayParams) {
                  if ([subParam isKindOfClass:[NSString class]]) {
                      [xml appendFormat:@"<string>%@</string>",subParam];
                       // urlBodyData = [urlBodyData stringByAppendingFormat:@"%@=%@&",param,arrayParams];
                    }
                }
                [xml appendFormat:@"</array></plist>"];
                urlBodyData = [urlBodyData stringByAppendingFormat:@"%@=%@&",param,xml];
            }
        }
