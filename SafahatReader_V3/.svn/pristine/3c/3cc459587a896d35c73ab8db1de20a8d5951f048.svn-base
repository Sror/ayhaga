...
@synthesize issuesArray,horizontalTableView,viewController,articlesArray; //articlesStillDownloading,
@synthesize delegate ;
...
-(IBAction)askForDeleteIssue:(UISwipeGestureRecognizer *)sender{
    ...
    if(![self isIssueDownloaded:(NSString *)[[self.issuesArray objectAtIndex:index] objectForKey:@"issue.id"]])
        return ;
...
{
    if ([self.issuesArray count]<sectionSize) {
        isLessThan4=YES;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(sectionSize-indexPath.row <=[self.issuesArray count])
    {
        ...
        dispatch_async(concurrentQueue, ^{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                NSString *thumbName=[NSString stringWithFormat:@"%@",[[self.issuesArray objectAtIndex:cellIndex] objectForKey:@"issue.thumb.url"]];
                NSData *thumbImg=[CacheManager getDataWithName:[NSString stringWithFormat:@"%@t%@",[[self.issuesArray objectAtIndex:cellIndex] objectForKey:@"image.lastmodified.date"],[[thumbName componentsSeparatedByString:@"/"] lastObject]] inRelativePath:@"Thumbnail"];
                if (thumbImg) {
                    [cell.thumbnail setImage:[UIImage imageWithData:thumbImg]];
                }
                else
                    [cell.thumbnail setImageWithURL:[NSURL URLWithString:thumbName] Date:[[self.issuesArray objectAtIndex:cellIndex] objectForKey:@"image.lastmodified.date"]];
                ...
                if([UsageData getiOSVersion] >= 7)
                {
                 ...
                 cell.progressView.transform =  CGAffineTransformMakeRotation(3.14);
                                ...
                                NSString *issueNumber = [[self.issuesArray objectAtIndex:cellIndex] objectForKey:@"issue.id"];
                ...
                /*     for (int i=0; i<[self.articlesStillDownloading count]; i++) {
                 ...
                 if ([[dict objectForKey:@"issue.id"] isEqual:[NSString stringWithFormat:@"%@",issueNumber]]) {
                 isDownloading=YES;
                 progressRate=[[dict objectForKey:@"progress"] floatValue];
                 break;
                 }
                 }
                 */
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    currentIssue = [self.issuesArray objectAtIndex:sectionSize-1-indexPath.row];
...
-(void)read:(NSIndexPath *)indexPath{
    ...
    issueName = [[self.issuesArray objectAtIndex:sectionSize-1-indexPath.row] objectForKey:@"issue.id"];
    NSString *issueNumnber = [[self.issuesArray objectAtIndex:sectionSize-1-indexPath.row] objectForKey:@"issue.id"];
    ...
    AVC.issueTitle= [[self.issuesArray objectAtIndex:sectionSize-1-indexPath.row] objectForKey:@"issue.name"];
    ...
    if (alertView.tag==-2) {
        ...
        if (!isDeleted) {
            if (buttonIndex == 1){
                ...
                {
                    //       [self.articlesStillDownloading addObject:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"%@",[[self.issuesArray objectAtIndex:alertView.tag] objectForKey:@"issue.id"]],@"issue.id",@"0.0",@"progress", nil]];
...
-(void)deleteIssueWithId:(NSString *)issueId{
    ...
    [CacheManager deleteItemsAtPath:[NSString stringWithFormat:@"%@",[[self.issuesArray objectAtIndex:issueId.intValue] objectForKey:@"issue.id"]]];
    NSString *key=[NSString stringWithFormat:@"%@",[[self.issuesArray objectAtIndex:issueId.intValue] objectForKey:@"issue.id"]];
    ...
    //
    NSDictionary *deletedBook = [NSDictionary dictionaryWithObjectsAndKeys:[[self.issuesArray objectAtIndex:issueId.intValue] objectForKey:@"issue.id"],@"issueID",[NSDate date] ,@"Time", nil];
...
-(void)downloadIssueWithId:(NSString *)issueId{
    ...
    issueId=[[self.issuesArray objectAtIndex:issueId.intValue] objectForKey:@"issue.id"];  //http://api.hindawi.org/v1/safahat/list/issue/1/articles/  best.cover.5x5/
